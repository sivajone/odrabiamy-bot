...
export async function scrape(
  ...
  try {
    ...
    try {
      ...
      const cookiesAccept = await webPage.waitForSelector(cookiesAcceptPath, {
        timeout: 5000,
      });
      log("4.a cookies accept found", timer);

      await cookiesAccept!.evaluate((node) =>
        (node as HTMLButtonElement).click()
      );
      await webPage.waitForSelector(cookiesAcceptPath, {
        hidden: true,
        timeout: 5000,
      });
    ...
    if (webPage.url() !== "https://odrabiamy.pl/moje") {
      ...
      interaction.channel?.send(
        "Pliki cookies wygasły, zalogowano się ponownie."
      );
    ...
    if (popupCloseElement) {
      await popupCloseElement.evaluate((node) =>
        (node as HTMLButtonElement).click()
      );
    ...
    try {
      await webPage.waitForResponse(
        (response) => response.url().includes("exercises"),
        {
          timeout: 5000,
        }
      );
    ...
    await webPage.waitForResponse((response) =>
    ...
    const individualExerciseBtns = await async.filter(
      exerciseBtns,
      async (btn) => {
    ...
    if (exerciseBtns.length === 0) {
      const subexercises = await webPage.$$(
        `[id^='qa-exercise-no-${exerciseParsed}']`
      );
      const ids = await async.map(
        subexercises,
        async (btn: ElementHandle<Element>) => {
    ...
    for (let i = 0; i < exerciseBtns.length; i++) {
      ...
      await exerciseBtns[i].evaluate((node) =>
...
function log(msg: string, timer: [number, number]): void {
  console.log(
    `[${parseFloat(process.hrtime(timer).join(".")).toFixed(3)}] ${msg}`
  );
}
